...
function preprocessImage(
  ...
  if (CONTRAST_STRETCH) {
    ...
    for (let i = 0; i < size; i++) {
      data[i * 4] = Math.max(
        0,
        Math.min(255, ((data[i * 4] - low) / span) * 255),
      );
      data[i * 4 + 1] = Math.max(
        0,
        Math.min(255, ((data[i * 4 + 1] - low) / span) * 255),
      );
      data[i * 4 + 2] = Math.max(
        0,
        Math.min(255, ((data[i * 4 + 2] - low) / span) * 255),
      );
    }
  }
...
function removeSmallComponents(
  ...
  const minArea = Math.max(
    1,
    Math.floor(totalFloor * MIN_COMPONENT_AREA_RATIO),
  );
...
function maskToRedOverlayDataUrl(
  mask: Uint8ClampedArray,
  width: number,
  height: number,
): string {
...
async function runSingleInference(
  ...
  if (dims.length === 4 && dims[1] === 1) {
    ...
    if (USE_HYBRID_AND && floorProb) {
      for (let i = 0; i < smallSize; i++) {
        floorMaskSmall[i] =
          classMap[i] === FLOOR_CLASS_INDEX &&
          floorProb[i] >= FLOOR_PROB_THRESHOLD
            ? 255
            : 0;
      }
...
export async function detectFloorWithSegFormer(
  imageDataUrl: string,
  ...
  floorMaskSmall = morphologicalClose(
    floorMaskSmall,
    outW,
    outH,
    MORPH_CLOSE_RADIUS,
